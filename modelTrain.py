...
def get_user_embedding(uid, user_age):
    print('---------------------------------3. get_user_embedding definition User embedding matrix')
    """
...
def get_user_feature_layer(uid_embed_layer, age_embed_layer):
    print('---------------------------------4. get_user_feature_layerFully connect Users embedded matrix together to generate User characteristics')
...
def get_isbn_embed_layer(isbn):
    print('---------------------------------5. get_isbn_embed_layer---Get the embedding vector of isbn')
...
def get_books_cnn_layer(book_title):
    print('---------------------------------9. get_books_cnn_layer---book_title text convolutional network implementation')
...
def get_books_feature_layer(isbn_embed_layer, year_of_publication_embed_layer, book_author_embed_layer,
                            publisher_embed_layer,
                            dropout_layer):
    print('---------------------------------10. get_books_feature_layer将books All layers are fully connected together')
    ...
    #embed_dim=32
    isbn_fc_layer = tf.keras.layers.Dense(embed_dim, name="isbn_fc_layer", activation='relu')(isbn_embed_layer)
    isbn_fc_layer is shape=(None, 1, 32);
    -------------------------------------------------
    ...
    These embedding layers are all shape=(None, 1, 32);
...
class mv_network(object):
    def __init__(self, batch_size=256):
        print('---------------------------------1.Build a neural network calculation graph---mv_network的__init__方法')
        ...
        # -----4.4 Set checkpoints when initializing；
    ...
    def compute_metrics(self, labels, logits):
        print('---------------------------------compute_metrics Calculation index')
  ...
  tensorflow2.0 has 3 ways to train the model:
     ...
     @tf.function Custom training loop; custom training loop does not need to compile the model, directly uses the optimizer to back-propagate the iterative parameters according to the loss function, with the highest flexibility。
    """
    @tf.function
    def train_step(self, x, y):
        print('---------------------------------13. train_step Training model')
        # ----------------------------------------1.tf.GradientTape gradient solution tool, using MSE (mean square error) algorithm; get gradient grads
        # tf.GradientTape gradient solving tool; GradientTape will monitor the trainable variables; in this way, the gradients of all trainable variables can be calculated, and then the next step is updated;
    ...
    def training(self, features, targets_values, epochs=5, log_freq=50):
        print('---------------------------------11. training Training model')
        ...
        for epoch_i in range(epochs):
            ...
            train_end = time.time()
            print(
                '\nThe {} epoch Training model ######({}steps)######耗时： {}'.format(epoch_i + 1, self.optimizer.iterations.numpy(),
    ...
    def testing(self, test_dataset, step_num):
        ...
        print('The loss of model testing is: {:0.6f} ###### Calculation index mae: {:0.6f}'.format(avg_loss.result(), self.ComputeMetrics.result()))
        # If the test finds that the loss of the model trained in this round is smaller, save it;
        if avg_loss.result() < self.best_loss:
            self.best_loss = avg_loss.result()
            print("best loss = {}".format(self.best_loss))
            print('================Less loss this round，save checkpoint===================')
...
if __name__ == '__main__':
    ...
    # --------------------------------------------5.Generate user feature matrix and save it locally
    # ------------------------------------5.1 Build user characteristics model
    user_layer_model = keras.models.Model(
        # The field order of users is：user_id	age
        inputs=[mv_net.model.input[0], mv_net.model.input[6]],
        outputs=mv_net.model.get_layer("user_combine_layer_flat").output)
    users_matrics = []
    for item in users.values:
        print('User characteristics item................', item)
        # -------------------------------5.2.Feed the data into the user feature model user_layer_model,
        #uid,user_age
        user_combine_layer_flat_val = user_layer_model([np.reshape(item.take(0), [1, 1]),
                                                        np.reshape(item.take(6), [1, 1])])
        # -------------------------------5.3.Combine the trained user features into a user feature matrix users_matrics and save it locally
        users_matrics.append(user_combine_layer_flat_val)
    pickle.dump((np.array(users_matrics).reshape(-1, 200)), open('users_matrics.p', 'wb'))
    print('###############################The user feature matrix is saved locally######################################')
